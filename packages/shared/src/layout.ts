...
export const ROOM_COUNT = FLOORS * ROOMS_PER_FLOOR // 21 — 7-room decision (AD-046)
...
export type RoomIndex = 1 | 2 | 3 | 4 | 5 | 6 | 7
export const ROOM_INDEXES = [1, 2, 3, 4, 5, 6, 7] as const

/** Travel-budget assumptions, roadmap step 0: halls ~30 tiles, rooms ~4 tiles. */
export const HALL_LENGTH_TILES = 30

/**
 * Room geometry (cycle 2.5, AD-010; re-derived AD-036; 7 rooms AD-046): the
 * 7 rooms of a guest floor are contiguous x-segments of ROOM_DEPTH_TILES
 * tiling [ROOM_HALL_START_TILES, ROOM_HALL_START_TILES + 7 × 3.25 = 24.75] —
 * an open hall at each end: ~2 tiles west of room 1 and a ~5.25-tile east
 * gap that clears the front-facing elevator door (80 px at the 32 px/tile
 * grid, AD-036 v3) from the last room's billboard. The grand lobby floor
 * has no rooms.
 */
