...
4. Grace state, per-room coverage breakdowns, and other players' private snapshots are
   never client-bound.
5. Audit rule: every message type carries a one-line comment naming its intended
   recipients. Reviewers and the Verifier grep every `send`/`broadcast` call against
   that list. If a task seems to require violating a rule above, STOP — it is a spec
   bug, not an implementation problem.

## Conventions

- Message types are past-tense domain events mirroring prd §6 behaviors:
  `room:prepped`, `room:trashed`, `elevator:called`, `player:fired`, …
