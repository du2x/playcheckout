...
The v1.6 insert (**3.E**, `stairs`, AD-040) also sits before 3.3: removing the
west elevator halves guest transport capacity, so 3.3's complaint-budget dials
must tune against the one-car economy, and the ambush's pinned kill check (an
ambush never creates a complaint) belongs to 3.3's trigger surface; 3.5's
balance gate models the saboteur's stun tool.
...
| 3.E | `stairs` | v1.6 stairs & ambush (AD-040, **inserted before 3.3** — 3.3's budget dials must tune against the one-car economy, and the ambush kill-check belongs to 3.3's trigger surface): the west elevator is replaced by a camera-free **stairwell** (3s/floor + 2s breath catch, staff-side only; entry/arrival observable, interior publishes nothing) and the saboteur gains the **ambush** — automatic, anonymous 20s stun on an opposite-direction stairs pass, no limiter; the single surviving elevator collapses all two-car machinery (AD-019/023/024/026/027 predicates, payloads keep `car: 1`); guests ride E only. Client: stairwell marker, stairs screen (transit/breath/stun), ambush toast + saboteur confirmation, single-car panels. Sim: `sim:stairs_one_car`, `sim:stairs_transit`, `sim:stairs_ambush` — plus the pinned kill check (an ambush never *creates* a complaint). Client: `client:stairs`. §8 v1.6 recompute lands in the cycle (one-car + stairs verdict in prd §8) | `sim:stairs_one_car`, `sim:stairs_transit`, `sim:stairs_ambush`, `client:stairs` |
...
| 3.5 | `guest-exit` | Rate-based bot sims, the new-signal proof, v1.5 edition: (a) staff bots reach `SETTLE_TARGET` against pure churn at the 6p cadence, (b) **mis-placement saboteur bot** (free wrong placements + interception) wins at plausible rates against interception-shaped staff — doubles as the v1.4/v1.5 §7 **balance gate**: if interception cannot keep up, dials move before §7 locks (and it calibrates `SETTLE_TARGET` + re-checks the shrunken complaint budget's reachability) — **proven 3.5 (AD-043): 6p 20/20, 5p 20/20, 4p 19/20; 6p mis-place 17/20 staff wins (bot 20–90% band, human 35–65% expected); `SETTLE_TARGET` locked at 5/7/9; budget holds (discovered <8 in 20/20, mode ≤2; ambush 0 complaints)** | `sim:guest_exit_a`, `sim:guest_exit_b` |
| 3.6 | `telemetry` | Phase exit (postponed from 2.11): JSONL telemetry with 1/s coverage sampling (FR-23) + guest extension (FR-23/24 — guest arrivals/check-ins/checkouts, suitcase carry/place/pickup + carry-clock expiries, complaint events with source + provenance, bleed-vs-throughput KPIs) + KPI computation from JSONL; the v1.2 exit bot sims re-proven under the full economy: (a) staff vs. AFK saboteur ≥80% pre-buzzer, (b) last-60s blitz defeats spread bots at plausible rates — **COMPLETE (AD-044): file `data/telemetry/<code>-<idx>.jsonl` git-ignored, KPI layer proven over 6 synthetic files (rounds 5, aborted 1, malformed 2, sabWin 0.4, correctAcc 0.6, catches 9.6/h, crime 0.94s, decoy 0.5, settle 7.6, complaints 1.2, carry 0.8, provenance 4/2), `sim:exit_a` 20/20 20/20 19/20, `sim:exit_b` 20/20 (relaxed band 8–20, delta 0, kill boxes preserved)** | `sim:telemetry`, `sim:telemetry_guests`, `sim:exit_a`, `sim:exit_b` |
