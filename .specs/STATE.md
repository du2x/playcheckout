...
## Handoff
- **Feature**: `guest-exit` (cycle 3.5, AD-043) — COMPLETE. T1 `a697185` pure-churn throughput harness `sim:guest_exit_a` · T2 `6db4f3e` mis-placement interception harness `sim:guest_exit_b` (room+1 on next guest floor + sameFloor correction + building-wide sab steal) · T3 `11f0a29` keep `SETTLE_TARGET` after 3.5 gate (probe proves 5/7/9) · T4 docs (prd §7/§8 + roadmap Phase 3 exit + AD-043).
- **Phase / Task**: Validate → pending. Next: Verifier `validation.md` PASS + `validate_state.py guest-exit` exit 0.
- **Gates**: typecheck ✓ · lint ✓ · test:sim 232 ✓ (guestExit 6) · test:client 111 unit ✓ · server 83 ✓
- **Next step**: cycle 3.6 `telemetry` (Phase exit — JSONL FR-23/24 + KPI + `exit_a`/`exit_b` re-proof under the full economy).
- **Blockers**: none.
- **Branch**: master
