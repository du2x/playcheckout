...
| Colyseus attach wiring (verified against 0.18.3 sources) | `WebSocketTransport({ noServer: true })` + `attachToServer(app.server)` + `createNodeMatchmakingMiddleware()` on Fastify's `onRequest` chain (hijack when the middleware answers) | Mirrors colyseus/vite's documented shared-HTTP-server pattern; the `{ server }` transport option mounts a competing express `request` listener that corrupts Fastify responses. Risk row above: resolved by source verification, boot test proves it |
