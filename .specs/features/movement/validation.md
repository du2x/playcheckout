...
| MOVE-08 | buzzer: positions persist (non-lobby floor included); lobby-phase movement allowed on all floors (AD-015) | `movement.test.ts:216-234` — positions persist across `lock()` on lobby floor; `:236-254` — player rides to `floor1` in lobby phase and walks 10 ticks to x=3; client `movement.spec.ts:243-252` — post-buzzer hold moves ada on floor1 | ✅ |
