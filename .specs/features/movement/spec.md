...
| Car selection on call | ~~the car that would serve it sooner… exact tie → car 1 (west)~~ **Superseded by AD-012**: among idle cars the one whose landing is closest to the caller's x is dispatched (tie → car 1); a call is ignored only when it duplicates an in-flight pickup at the caller's floor with the same destination (or is already queued) — the panel flashes either way (FR-5 decoy rule, narrowed) | prd leaves the assignment rule open; playtest showed the flat tie rule starves car 2 and destination-only decoys swallow valid calls | y (user, AD-012) |
