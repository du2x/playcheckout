...
| Caller never boards | Car arrives, opens doors, dwell, nobody boards → idles open-doors there; ~~same-floor re-call = decoy flash (duplicate pickup)~~ **amended by AD-019 (2026-08-29)**: a parked open-doors car no longer duplicates — the call summons the OTHER car to this floor; only when BOTH cars are parked here does the re-call stay a decoy flash. Calls from other floors dispatch normally. **Amended by AD-023 (2026-08-30)**: a call pressed AT the parked car's own landing is pinned to that car — decoy flash, the other car is never summoned (mid-hall callers keep the AD-019 behavior). **Amended by AD-025 (2026-08-30)**: proximity auto-boarding is disabled — a call pressed at the parked car's own landing now BOARDS the caller (outranking the duplicate predicate); the both-parked decoy flash remains mid-hall-only | Only coherent reading now that calls carry no destination; landings pin (hall-button model) | y |
...
7. IF a call arrives for a pickup floor a car is already en route to (or queued for) THEN the system SHALL emit the `elevator:called` flash without a new dispatch (duplicate predicate = pickup floor only, narrowing AD-012; **AD-019**: a mid-hall caller with a car parked open-doors at the pickup floor summons the OTHER car instead, and only both-cars-parked flashes; **AD-023**: a caller standing at a landing pins the call to that landing's car — it dispatches when idle, queues pinned when busy, and a call at the parked car's own landing stays a decoy flash) <!-- unwanted-behavior -->
8. WHILE a car is moving THEN the system SHALL reject walk intents from its riders; WHILE doors are open (dwell or idle) THEN riders SHALL be able to walk off and — **amended by AD-025 (2026-08-30)**: boarding is the explicit call press at the landing (proximity auto-boarding disabled; capacity 2 and the queued-call drop unchanged; ~~closest first, ties by playerId, overflow queues~~ applies to proximity candidates only, which no longer exist) <!-- state-driven -->
...
5. ~~IF a call is made from the floor where a car is idling with open doors THEN the system SHALL treat it as a duplicate (decoy flash, no dispatch) — boarding and pressing is the way to move it~~ **Amended by AD-019**: a single parked car no longer duplicates for a mid-hall caller — the call dispatches (or queues for) the OTHER car; the flash remains when BOTH cars are parked at the floor, where boarding/pressing is still the way to move them. **Amended by AD-023**: a call pressed at the parked car's own landing is pinned to it — decoy flash, no summon of the other car <!-- unwanted-behavior -->
...
- ~~IF a rider exits a car with open doors THEN they SHALL NOT re-board that car until the car next departs (door-open-episode guard) — exiting is final for the stop; exit itself is available in any phase~~ **Superseded by AD-025 (2026-08-30)**: with proximity auto-boarding disabled there is no board/exit oscillation — the guard is deleted; an exiter re-boards by pressing the call again at any distance within the landing zone (confinement applies to hallway walking after exit)
