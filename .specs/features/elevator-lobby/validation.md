...
| EL-04 | Lobby-phase walking on guest floors is allowed (AD-015): pre-round exiter can walk away from the landing | `packages/sim/src/movement.test.ts:761-785` — after pre-round arrival on `floor1`, exit succeeds, 25 ticks of walking move `lastX` > 0, and the episode guard still prevents re-boarding the same stop. Harness: `elevatorLobby.spec.ts:65-67` — `#round-hud` never mounts (no round ever started). Confinement guard removed at `packages/sim/src/movement.ts:178` | PASS |
