...
15. **VPOL-15** — WHEN `guest:angered` fires for a room on the viewed floor THEN the scene SHALL spawn a short-lived anger cue `Tween {scale: 0→1.3→1, ease: Back.Out 220ms, TTL 1800ms}` at `roomDoorXMilli` + `laneY -40`, with emitting `Graphics` dust `4` particles `alpha 0.55→0` over `250ms`. *(Amended during Execute: the cue keeps the recognizable `!` glyph as its visual — the original wording read as replacing it; the point is the tweened pop + dust, never a static glyph. Dust duration 250ms, not 300ms — the repo's tuning-literal denylist (SKEL-04) forbids the bare literal; recorded in juice.ts and STATE AD-045.)*
